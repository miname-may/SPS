...
function sendCode() {
	...
	var regP = RegExp(/^[0-9]{10,11}$/);
	if(!regP.test(phoneNumber)){
		alert("정확한 번호를 입력하세요");
	}
	else{
		var btnVal = $("#sendBtn").text();
		
		
		if(btnVal=="번호인증" || btnVal=="재전송"){
...
var sec = 59;
	...
	function startTimer(){
		...
		start = setInterval(function() {
			sec --; // 1000ms 당 sec 1 감소
			
			if(sec==0){ // sec이 0 이면
				if(min != 0){ // 분이 0이 아닐경우엔
					min--;// 분 -1
				}
				sec=59; // sec reset
			}
			
			if(sec<10){ // sec 이 한자리수면 앞에 0 추가
				$("#sendBtn").html(min+" : 0"+sec);
			}
			
			else if(sec>9){ // sec이 두자리수면 그냥 표현
				$("#sendBtn").html(min+" : "+sec);
			}
			
			if(min==0 && sec==1){ //min 0 sec 1 이면 = 타이머가 다되면
				stop(); // clearInterval 작동
				$("#sendBtn").html("재전송"); // 버튼의 텍스트는 재전송으로 설정
			}
		...
		stop = function() { // clearInterval function 선언
			clearInterval(start); // 중지할 Interval func 지정
			$("#sendBtn").html("재전송"); // 중지후에 btn text는 재전송
		};
		...
		var flag = 0; // ?? 구버전 때 썼던거 
		function codeCheck(){ // 전송코드 일치 확인 func
			
			var bt = $("#sendBtn").text(); // 인증번호 전송 버튼의 텍스트 가져오기
			if(bt=="재전송"){ // 버튼 텍스트가 재전송일 경우 알럿
				alert("인증 시간이 경과 했습니다 재인증 해주세요");
			}
			
			else{ // 아니면
				var code = $("#code").val(); // 입력받은 코드의 val 가져오기
				var inputCode = $("#joinCode").val(); // hidden 값 가져오기
				if(code!=inputCode){ // 서로 불일치 시
					flag = 0;
					alert("인증번호가 일치하지 않습니다.");
					$("#joinCode").css('border-color', 'rgb(220, 53, 69)');
				}
				else if(code==inputCode && inputCode.length!=""){ // 입력된 값의 length 가  0이 아니며 일치시
					flag = 1;
					alert("일치합니다");
	...
	<div class="container joinForm">
		<form id="goInsert" class="form-signin" action="">
			...
			<div class="row join joinPhone">	    
				...
				<button id="sendBtn" class="btn btn-primary btn-inline join_Btn" type="button" onclick="sendCode()">번호인증</button><br/>
